return function(busted, options)
  ...
  local function filterExcludeTags(name)
  ...
  local function filterTags(name)
  ...
  local function filterOutNames(name)
  ...
  local function excludeNamesFile(name)
  ...
  local function name(nm)
    for _, candidate in pairs(options.name) do
      if candidate:find(getFullName(nm), 1, true) then
        return nil, true
      end
    end
  ...
  local function filterNames(nm)
    for _, filter in pairs(options.filter) do
      if getFullName(nm):find(filter) ~= nil then
        return nil, true
      end
    end
  ...
  local function printTestName(element, _parent, status)
