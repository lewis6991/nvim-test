...
s:set(
  ...
  'オブジェクトの内容が同一であることが期待されています。\n実際の値:\n%s\n期待されている値:\n%s'
)
s:set(
  'assertion.same.negative',
  'オブジェクトの内容が同一でないことが期待されています。\n実際の値:\n%s\n期待されていない値:\n%s'
)

s:set(
  'assertion.equals.positive',
  'オブジェクトが同一であることが期待されています。\n実際の値:\n%s\n期待されている値:\n%s'
)
s:set(
  'assertion.equals.negative',
  'オブジェクトが同一でないことが期待されています。\n実際の値:\n%s\n期待されていない値:\n%s'
)

s:set(
  'assertion.unique.positive',
  'オブジェクトがユニークであることが期待されています。:\n%s'
)
s:set(
  'assertion.unique.negative',
  'オブジェクトがユニークでないことが期待されています。:\n%s'
)
...
s:set(
  ...
  '回呼ばれることを期待されていますが、実際には%s回呼ばれています。'
)
s:set(
...
s:set(
  ...
  '関数が期待されている引数で呼ばれていません'
)
s:set(
  'assertion.called_with.negative',
  '関数が期待されている引数で呼ばれています'
)

s:set(
  'assertion.returned_arguments.positive',
  '期待されている返り値の数は%sですが、実際の返り値の数は%sです。'
)
s:set(
  'assertion.returned_arguments.negative',
  '期待されていない返り値の数は%sですが、実際の返り値の数は%sです。'
)

-- errors
s:set(
  'assertion.internal.argtolittle',
  '関数には最低%s個の引数が必要ですが、実際の引数の数は: %s'
)
s:set(
  'assertion.internal.badargtype',
  'bad argument #%s: 関数には%s個の引数が必要ですが、実際に引数の数は: %s'
)
