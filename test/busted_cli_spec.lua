...
describe('busted.modules.cli', function()
  ...
  it('applies defaults and aliases when no args are provided', function()
    ...
    assert(parsed['auto-insulate'])
    assert(parsed['keep-going'])
    assert(parsed.k)
  end)
  ...
  it('honors negatable flags', function()
    ...
    assert(not parsed['auto-insulate'])
    assert(parsed.lazy)
    assert(not parsed['keep-going'])
    assert(not parsed.k)
    assert(not parsed.recursive)
    assert(not parsed.R)
    assert(parsed.list)
    assert(parsed.l)
  end)
  ...
  it('toggles both sort flags via combined options', function()
    ...
    assert(sorted['sort-files'])
    assert(sorted['sort-tests'])

    local unsorted = parse_args({ '--no-sort' })
    assert(not unsorted['sort-files'])
    assert(not unsorted['sort-tests'])
  end)
end)
